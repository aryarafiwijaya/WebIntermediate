 ...
 * @param {string} token - Bearer token user login
 * @param {PushSubscription} subscription - Hasil dari registration.pushManager.subscribe()
 */
export async function subscribeNotification(token, subscription) {
  const { endpoint, keys } = subscription.toJSON();

  return safeFetch(ENDPOINTS.NOTIFICATION_SUBSCRIBE, {
    method: 'POST',
    headers: {
 ...
 * @param {string} token - Bearer token user login
 * @param {string} endpoint - Endpoint dari subscription
 */
